window.ActionModal = (function () {
  ...
  // Repository-intelligence block (conventions + import graph, generated by
  // conventions-cli on session create and cached in main). '' until ready —
  // callers append it conditionally. Never throws.
  async function repoIntelFor(taskId) {
    try {
      var task = AppState.tasks.get(taskId);
      if (!task || !task.worktreePath) return '';
      var res = await window.klaus.task.getRepoIntel(task.worktreePath);

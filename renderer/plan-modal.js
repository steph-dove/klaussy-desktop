window.ActionModal = (function () {
  ...
  var PLAN_PROMPT = [
    ...
    '',
    'Launch 2–3 explore subagents IN PARALLEL via the Agent tool. Use specialized subagents when available (`subagent_type: code-explorer`); fall back to `general-purpose`. Each agent should target a different angle:',
    '- Similar features: "Find features in this codebase that already do something analogous to this task. Trace the implementation comprehensively (entry → core logic → output). Return file:line refs and the 5–10 most important files."',
    '- Architecture / conventions: "Map the architecture for the area this task touches — entry points, abstraction layers, IPC handlers, key utilities. Identify existing patterns and conventions. Return file:line refs."',
    '- (Optional, when relevant) UI / testing patterns: "Identify UI patterns, testing approaches, or extension points relevant to this task."',
    '',
    ...
    '',
    '## Phase 3 — Design (parallel architectures)',
    '',
    'Launch 2–3 architect subagents IN PARALLEL with different priorities (`subagent_type: code-architect` if available; otherwise `general-purpose`). Pass each the user\'s task, the answers from Phase 2, and the file list from Phase 1:',
    '- "Minimal change" — smallest diff, maximum reuse of existing code, fewest new files.',
    '- "Clean architecture" — clear abstractions, ergonomic for future change. May refactor more.',
    '- "Pragmatic balance" — fast and good enough.',
    '',
    'Each agent should return a concrete blueprint: files to create/modify, component responsibilities, data flow, and a phased build sequence.',
    '',
    'Present the user a brief summary of each, the trade-offs, and your recommendation with reasoning. Ask which they want.',
    '',
    ...
    '- Simplicity / DRY / readability (`subagent_type: code-reviewer` if available).',
    '- Bugs / silent failures / inadequate error handling (`subagent_type: silent-failure-hunter` if available — flag every empty `catch` on a user-initiated action).',
    '- Project conventions and the anti-patterns below.',
    '',
    ...
    '## Phase 7 — Summary',
    '',
    'Write a 3–5 line summary: what was built, key decisions, files modified, suggested next steps.',
    '',
  ...
  var ACTIONS = {
    plan: {
      ...
      title: 'Plan a task',
      submitLabel: 'Plan',
      hint: 'Provide details. A new Claude tab will open on this worktree and run a multi-agent plan flow (parallel exploration → clarify → parallel architectures → approve → implement → review) — all local, no cloud round-trip.',

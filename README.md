...
> **📣 Out of stealth.** Klaussy has been in heads-down development for six months — quietly battle-tested by a private group of developers shipping real production work through it every day. After months of dogfooding, refinement, and distributing builds to testers, it's finally open to everyone.
...
*   **⚡ Parallel Agents, Isolated Worktrees**
    One task = one git worktree + one independent agent. Run Claude Code, Codex, Gemini, Antigravity, Copilot, Cursor, Cline, and opencode side by side — or drive a tab with a local model through Ollama (via Aider), or drop to a plain shell. The visual sidebar tracks staged/unstaged counts, status, and branch alignment per task.
...
*   **🔄 Cross-Agent Session Resuming**
    Start a session with Claude Code, pause it, and resume it under Gemini, Antigravity, Cursor, Cline, or opencode. Klaussy Desktop automatically compiles a structured handoff brief to catch the new agent up.
...
3.  **An Agent CLI or IDE extension** (Claude Code is the default; also supports Codex, Gemini, Antigravity, Copilot, Cursor, Cline, opencode, and local models via Ollama + Aider)

...
*   **⚡ Parallel Agents, Isolated Worktrees**
    One task = one git worktree + one independent agent. Run Claude Code, Codex, Gemini, and Copilot side by side. The visual sidebar tracks staged/unstaged counts, status, and branch alignment per task.
...
*   **🔄 Cross-Agent Session Resuming**
    Start a session with Claude Code, pause it, and resume it under Gemini or Cursor. Klaussy Desktop automatically compiles a structured handoff brief to catch the new agent up.
...
3.  **An Agent CLI** (Claude Code is the default)

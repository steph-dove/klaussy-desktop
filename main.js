...
//     check. loadConfig is a real module import now; instances is the live
//     Map from state/instances.js.
//   - instances.spawnInWorktree needs to see `isQuitting` (moves with
//     bootstrap/app-events.js in Phase 4) and `startCIPolling` (moves with
//     state/ci-poll.js in Phase 2 step 9). startCIPolling is a hoisted
//     function declaration below so the direct reference resolves when the
//     arrow runs, not when setDeps is called.
pathGate.setDeps({ loadConfig, getInstances: () => instances });
instancesModule.setDeps({
  isQuitting: () => isQuitting,
  startCIPolling: (id, wt, br) => startCIPolling(id, wt, br),
});
